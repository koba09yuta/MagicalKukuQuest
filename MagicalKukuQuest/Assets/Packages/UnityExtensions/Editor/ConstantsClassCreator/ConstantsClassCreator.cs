...
public static class ConstantsClassCreator
{
    ...
    public static void Create<T>(string nameSpace, string className, string classInfo, Dictionary<string, T> valueDic)
    {
        //入力された型の判定
        ...
        {
            Debug.Log(className + ".cs" + "の作成に失敗しました.想定外の型" + typeof(T).Name + "が入力されました");
            return;
        }
        ...
        builder.Append(indent).AppendFormat("/// {0}", classInfo).AppendLine();
        builder.Append(indent).AppendLine("/// </summary>");
        builder.Append(indent).AppendFormat("public static class {0}", className).AppendLine();
        ...
        foreach (KeyValuePair<string, T> valuePair in newValueDic)
        {
            ...
            builder
                .Append(indent)
                .Append(indent)
                .AppendFormat(@"  public const {0} {1} {2} ", typeName, valuePair.Key, EqualStr);
        ...
        //書き出し、ファイル名はクラス名.cs
        string exportPath = "Assets/Scripts/Constants/" + className + ".cs";
